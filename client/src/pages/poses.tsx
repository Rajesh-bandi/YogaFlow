...
interface YogaPose {
  ...
  const PosesPage = () => {
    ...
    // Fetch real yoga poses from the backend
    ...
    image: pose.image
      ? `/yoga/${pose.image.replace(/^\/api\/yoga\//, '').replace(/^\/yoga\//, '')}`
      : `https://source.unsplash.com/400x300/?yoga,${encodeURIComponent(pose.name)}`
    }));
